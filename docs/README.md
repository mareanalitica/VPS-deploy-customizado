...
* 🔒 **[Security & VPS Hardening](./infrastructure/security-passwords.md)** — Firewall rules (UFW), Fail2ban brute-force protection, Docker Swarm Secrets, and `.env` UUID backup policy.
* 🧭 **[Client Onboarding Runbook](./infrastructure/client-onboarding-runbook.md)** — Step-by-step: provision a new client's dedicated VPS with Terraform + Ansible.
* 🔑 **[SSH First Access](./infrastructure/ssh-first-access.md)** — Setting up key-based SSH access on a new or freshly-reset VPS, before running Ansible.
* 🔁 **[VPS Restart Cheatsheet](./infrastructure/vps-restart-cheatsheet.md)** — What to do after a reboot or a full OS reset; known gotchas from real debugging.
...
* 📄 **[ADR 0004: Terraform + Ansible Replication](./adr/0004-terraform-ansible-replication.md)** — Dedicated-VPS-per-client provisioning, why Hetzner, and known gaps.
* 📄 **[ADR 0005: Docker Swarm Secrets](./adr/0005-docker-swarm-secrets.md)** — Per-service secrets migration, confidence levels, and the deliberately deferred n8n case.
