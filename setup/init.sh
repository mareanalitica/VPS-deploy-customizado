...
if ! command -v docker &>/dev/null; then
    ...
    # harden-vps.sh so consegue proteger a chain DOCKER-USER (que o Docker usa
    # para ignorar as regras do UFW) DEPOIS que o Docker existe. Como ele
    # roda antes deste script, ele pula essa etapa na primeira execucao -
    # reaplicamos aqui, agora que o Docker acabou de ser instalado.
    HARDEN_SCRIPT="$(cd "$(dirname "$0")" && pwd)/security/harden-vps.sh"
    if [ "$EUID" -eq 0 ] && [ -f "$HARDEN_SCRIPT" ]; then
        echo "🛡️  Reaplicando harden-vps.sh agora que o Docker esta instalado (protege a chain DOCKER-USER)..."
        bash "$HARDEN_SCRIPT"
    fi
fi
